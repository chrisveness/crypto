/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
/* SHA-3 (FIPS 202) ‘Keccak’ reference implementation in JavaScript        (c) Chris Veness 2016  */
/*                                                                                   MIT Licence  */
/* www.movable-type.co.uk/scripts/sha3.html                                                       */
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */

'use strict';
/* eslint-env es6 *//* for arrow functions */
 ...
 * It is not at all optimised so is not recommended for production use, though it is fully tested
 * and could be used where performance is not a prime consideration.
 *
 ...
 */
Sha3.hash224 = function(message, options) {
 ...
 * @returns {string} Hash as hex-encoded string.
 */
Sha3.hash256 = function(message, options) {
 ...
 * @returns {string} Hash as hex-encoded string.
 */
Sha3.hash384 = function(message, options) {
 ...
 * @returns {string} Hash as hex-encoded string.
 */
Sha3.hash512 = function(message, options) {
    return Sha3.keccak1600(576, 1024, message, options);
};


Sha3.hash512.defaults = {
    padding:   'sha-3',
    msgFormat: 'string',
    outFormat: 'hex',
};
 ...
 * @private
 */
Sha3.keccak1600 = function(r, c, M, options) {
    var opt = Object.assign({}, Sha3.defaults, options);

    var l = c / 2; // message digest output length in bits
    ...
    var w = 64; // for keccak-f[1600]
    var blocksize = r / w * 8; // block size in bytes (≡ utf-8 characters)

    for (var i=0; i<msg.length; i+=blocksize) {
        for (var j=0; j<r/w; j++) {
            var lo = (msg.charCodeAt(i+j*8+0)<< 0) + (msg.charCodeAt(i+j*8+1)<< 8)
                   + (msg.charCodeAt(i+j*8+2)<<16) + (msg.charCodeAt(i+j*8+3)<<24);
            var hi = (msg.charCodeAt(i+j*8+4)<< 0) + (msg.charCodeAt(i+j*8+5)<< 8)
 ...
 * @private
 */
Sha3.keccak_f_1600 = function(a) {

    var nRounds = 24; // number of rounds nᵣ = 12 + 2ℓ, hence 24 for Keccak-f[1600] [Keccak §1.2]
    ...
    for (var r=0; r<nRounds; r++) {
        ...
        for (var x=0; x<5; x++) {
            ...
            var hi = C[(x+4)%5].hi ^ ROT(C[(x+1)%5], 1).hi;
            var lo = C[(x+4)%5].lo ^ ROT(C[(x+1)%5], 1).lo;
        ...
        // χ [Keccak §2.3.1
        for (var y=0; y<5; y++) {
            var C = [];  // take a copy of the plane
            for (var x=0; x<5; x++) C[x] = a[x][y].clone();
            for (var x=0; x<5; x++) {
 ...
 *
 * Note that until ES2015 modules are available, this gets added to the global namespace :(
 */

Sha3.Long = function(hi, lo) {
    this.hi = hi;
    this.lo = lo;
};

/**
 * Construct Long frm string representation.
 */
Sha3.Long.fromString = function(str) {
 ...
 * Copy 'this' Long.
 */
Sha3.Long.prototype.clone = function() {
 ...
 */
Sha3.Long.prototype.rotl = function(n) {
    if (n < 32) {
        var m = 32 - n;
        var lo = this.lo<<n | this.hi>>>m;
        var hi = this.hi<<n | this.lo>>>m;
        return new Sha3.Long(hi, lo);
    }
    if (n == 32) {
    ...
    if (n > 32) {
        n -= 32;
        var m = 32 - n;
        var lo = this.hi<<n | this.lo>>>m;
        var hi = this.lo<<n | this.hi>>>m;
        return new Sha3.Long(hi, lo);
    }
};
 ...
 */
Sha3.Long.prototype.toString = function() {
